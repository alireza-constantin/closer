...
| GO-02      | pgx/pgxpool, direct LISTEN configuration, sqlc wiring, transaction helper, local test harness; depends GO-01 | normal and direct realtime URLs validate independently; pool/connection lifecycle and transaction tests pass; sqlc query source is `db/queries` and schema input is ordered `db/schema`; generated output is `internal/postgres/sqlc`; test DB accepts only local `closer_test`; no domain command or production schema mutation.                                                                                                                                                                                                                          |
| GO-03A     | anonymous session and Admin-safe actor resolution; depends GO-02                                             | explicit anonymous POST creates one opaque cookie/session and no Participant; all GET/prefetch/public link reads are mutation-free; invalid/expired/revoked session is rejected; Admin actor never resolves/creates Participant; cookie, CSRF/origin, logout, session cleanup, and `closer_test` integration tests pass.                                                                                                                                                                                                                                   |
| GO-03B     | credentials, direct registration, anonymous upgrade, login, consumer limits; depends GO-03A                  | Argon2id/hash policy tests pass; email normalization/uniqueness races converge; upgrade preserves the same auth-user UUID; direct signup creates no Participant; login never merges a pre-existing anonymous browser; enumeration-safe invalid credentials, current logout, logout-all, session renewal/revocation, and consumer rate-limit tests pass. When GO-04 adds Participant, the unchanged auth-user UUID preserves Participant and Pair ownership without migration. Consumer reset delivery remains absent and explicitly tested as unavailable. |
...
| GO-03D     | Go executable schema alignment; depends GO-03C                                                               | UUID Go auth identity is separated from legacy Better Auth text IDs; ordered bootstrap schema includes Participant/Pair constraints; closer_test-only reset/apply tooling and docs pass; no domain behavior is added.                                                                                                                                                                                                                                                                                                                                      |
| GO-04      | Participant and Pair foundations; depends GO-03D                                                             | onboarding/access/status/create/name-edit/multi-Space commands and actor-relative projections port with existing domain constraints and idempotency; auth upgrade preserves Participant and Pair ownership.                                                                                                                                                                                                                                                                                                                                                |
...
Drizzle is the semantic reference for Closer domain behavior. Go owns its
executable rewrite schema, including UUID auth identity and Participant FK;
legacy Better Auth text IDs remain only in the Next reference. The runtimes
compare observable behavior and never share physical auth/Participant rows.
Ordered `apps/api/db/schema/` DDL bootstraps local rewrite and `closer_test`
before launch. A proper versioned production migration baseline must be
created and verified before real users exist. No Go schema is installed with
Drizzle `db:push`.
...
| GO-02      | `apps/api/db/schema`, `db/queries`, `sqlc.yaml`, `internal/postgres`           | pre-launch schema bootstrap, pgx tx/lock helpers, no behavior drift  | schema diff, lock smoke tests, no `db:push`                  |
| GO-03      | `internal/auth`, Go auth schema/queries, auth contract tests                   | session/credential/Admin identity; same-UUID credential upgrade      | auth/bootstrap/recovery/rate-limit parity                    |
