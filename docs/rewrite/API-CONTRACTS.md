...
| `POST /api/pairs/:pairId/private-rounds/:roundId/progress`                                     | immutable Conversation creator after completion or Decline | `{action,category?,clientRequestId}` | same-lane candidate or chosen-lane candidate/waiting/exhaustion; no Round is auto-Asked; `private.changed`                       |
