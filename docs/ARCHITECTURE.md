...
- **Private rounds** own independent question instances, participant answer submissions, mutual reveal, reactions, short replies, and lightweight active-question resumption.
...
## 7. Private round lifecycle

Multiple distinct Private rounds may be active concurrently for one pair. Each round independently owns its pair, question, initiating participant, participant answers, answer-derived reveal readiness, per-participant reveal-view timing, reactions, optional replies, and timestamps. Authorization and reveal are evaluated per round; no pair-wide `current private round` exists.
...
V1 does not persist a separate paused state. Leaving a round simply keeps it active. Within an active round, Pair Home or another lightweight active-question surface derives the participant-relative view:
...
Participants may resume unfinished rounds, revisit rounds while waiting, open any reveal-ready round, and start another eligible question without first revealing or acknowledging another round. Reveal-ready items should be visually easy to find, but they are not a pair-wide lock. This surface remains question-based and must not acquire chat-inbox, arbitrary-message, nested-thread, typing-indicator, unread-message, or folder behavior.

Supporting multiple rounds does not authorize accidental duplicate creation from double submission, request retry, or concurrent retry of the same user action. Creation should be safely idempotent where appropriate, without enforcing uniqueness across all active rounds or prescribing a particular idempotency mechanism.
...
- A participant has at most one reaction per round and may change or remove only their own reaction.
...
- Reactions and replies require an authorized participant and a reveal-ready round; ownership checks restrict changes or removal to that participant's own reaction or reply.
...
- Multiple distinct active Private rounds can coexist for one pair.
