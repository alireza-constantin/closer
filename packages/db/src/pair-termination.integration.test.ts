...
async function createGuestAuthUser(displayName: string) {
  ...
  await db.insert(user).values({
    id: authUserId,
    name: displayName,
    email: `${authUserId}@termination.closer.invalid`,
    isAnonymous: true,
  });
  return authUserId;
}
...
async function createJoinedPair() {
  ...
  const created = await createPairForParticipant(db, {
    ...
    intendedPersonName: "Second at termination",
...
async function askCurrentCandidate(
  pairId: string,
  participantId: string,
  category: "deep" | "fun",
) {
  const conversation = await startOrResumePrivateConversation(db, {
    pairId,
    participantId,
    category,
...
test("either claimed member can Unpair atomically and repeated termination is stable", async () => {
  ...
  const candidate = await startOrResumePrivateConversation(db, {
    pairId,
    participantId: first.id,
    category: "fun",
    clientRequestId: randomUUID(),
  ...
  expect(
    memberships.every(
      (membership) => membership.endedAt !== null && membership.endedDisplayName !== null,
    ),
  ).toBe(true);
  expect(
    memberships.find((membership) => membership.participantId === first.id)?.endedDisplayName,
  ).toBe("First at termination");
  expect(
    memberships.find((membership) => membership.participantId === second.id)?.endedDisplayName,
  ).toBe("Second at termination");
  await db
    .update(participant)
    .set({ displayName: "Renamed after termination" })
    .where(eq(participant.id, first.id));
  expect(
    (
      await db
        .select()
        .from(pairMembership)
        .where(and(eq(pairMembership.pairId, pairId), eq(pairMembership.participantId, first.id)))
    )[0]?.endedDisplayName,
  ).toBe("First at termination");
  expect(
    await db
  ...
  expect(
    (await db.select().from(rejoinInvite).where(eq(rejoinInvite.pairId, pairId)))[0]?.revokedAt,
  ).not.toBeNull();
  ...
  expect(
    await capture(startTogetherSession(db, { pairId, participantId: first.id, category: "deep" })),
  ).toBeInstanceOf(CloserDomainError);
  expect(
    await capture(
  ...
  const fresh = await createPairForParticipant(db, {
    participantId: first.id,
    intendedPersonName: "Second at termination",
    relationshipType: "partner",
  });
  pairIds.push(fresh.pair.id);
  const freshInvite = await issueOrReuseInitialInvite(db, {
    pairId: fresh.pair.id,
    participantId: first.id,
  });
  if (freshInvite.state !== "issued")
    throw new Error("Expected a fresh invitation for the new Pair.");
  await redeemInitialInvite(db, { token: freshInvite.token, participantId: second.id });
  expect(fresh.pair.id).not.toBe(pairId);
  const freshHistory = await getFormerEraHistoryForParticipant(db, {
    pairId: fresh.pair.id,
    participantId: first.id,
  });
...
test("an unclaimed member can End this space without manufacturing a second participant", async () => {
  ...
  const created = await createPairForParticipant(db, {
    participantId: first.id,
    intendedPersonName: "Intended only",
  ...
  const memberships = await db
    .select()
    .from(pairMembership)
    .where(eq(pairMembership.pairId, created.pair.id));
  expect(memberships).toHaveLength(1);
  expect(memberships[0]).toMatchObject({
    participantId: first.id,
    endedDisplayName: "Solo at termination",
  });
  expect(
    (await db.select().from(initialInvite).where(eq(initialInvite.pairId, created.pair.id)))[0]
      ?.revokedAt,
  ).not.toBeNull();
  ...
  expect(
    await capture(
      redeemInitialInvite(db, {
        token: invite.token,
        participantId: await createGuest("Late claimant"),
      }),
    ),
  ).toBeInstanceOf(CloserDomainError);
  expect(
...
test("termination and answer serialization retain only the transaction that commits first", async () => {
  ...
  const history = await getFormerEraHistoryForParticipant(db, { pairId, participantId: first.id });
  const round = history.eras[0]?.privateConversations
    .flatMap((conversation) => conversation.rounds)
    .find((item) => item.id === asked.roundId);
...
test("an answer committed before termination remains mutually visible in Former-Pair history", async () => {
  ...
  const firstHistory = await getFormerEraHistoryForParticipant(db, {
    pairId,
    participantId: first.id,
  });
  const secondHistory = await getFormerEraHistoryForParticipant(db, {
    pairId,
    participantId: second.id,
  });
  const roundFor = (history: typeof firstHistory) =>
    history.eras[0]?.privateConversations
      .flatMap((conversation) => conversation.rounds)
      .find((round) => round.id === asked.roundId);
...
test("termination committed before an answer preserves author-only Former-Pair history", async () => {
  ...
  const roundFor = (history: typeof firstHistory) =>
    history.eras[0]?.privateConversations
      .flatMap((conversation) => conversation.rounds)
      .find((round) => round.id === asked.roundId);
  expect(roundFor(firstHistory)?.answers.map((answer) => answer.body)).toEqual([
    "Only first answer",
  ]);
...
test("termination serializes initial claim in both commit orders and never lets a credential revive a Pair", async () => {
  ...
  const raced = await createPairForParticipant(db, {
    participantId: first.id,
    intendedPersonName: "Claim race claimant",
    relationshipType: "partner",
  });
  pairIds.push(raced.pair.id);
  const racedInvite = await issueOrReuseInitialInvite(db, {
    pairId: raced.pair.id,
    participantId: first.id,
  });
  ...
  const racedMemberships = await db
    .select()
    .from(pairMembership)
    .where(eq(pairMembership.pairId, raced.pair.id));
  ...
  const claimFirst = await createPairForParticipant(db, {
    participantId: first.id,
    intendedPersonName: "Claim first",
    relationshipType: "friend",
  });
  pairIds.push(claimFirst.pair.id);
  const claimFirstInvite = await issueOrReuseInitialInvite(db, {
    pairId: claimFirst.pair.id,
    participantId: first.id,
  });
  ...
  expect(
    (
      await db.select().from(pairMembership).where(eq(pairMembership.pairId, claimFirst.pair.id))
    ).every((membership) => membership.endedAt !== null),
  ).toBe(true);

  const terminationFirst = await createPairForParticipant(db, {
    participantId: first.id,
    intendedPersonName: "Termination first",
    relationshipType: "partner",
  });
  pairIds.push(terminationFirst.pair.id);
  const terminationFirstInvite = await issueOrReuseInitialInvite(db, {
    pairId: terminationFirst.pair.id,
    participantId: first.id,
  });
  ...
  expect(
    await capture(
      redeemInitialInvite(db, { token: terminationFirstInvite.token, participantId: claimant.id }),
    ),
  ).toBeInstanceOf(CloserDomainError);
  expect(
    await db
      .select()
      .from(pairMembership)
      .where(eq(pairMembership.pairId, terminationFirst.pair.id)),
  ).toHaveLength(1);
});

test("termination serializes guest replacement in both commit orders", async () => {
  const raced = await createJoinedPair();
  const racedInvite = await issueRejoinInvite(db, {
    pairId: raced.pairId,
    participantId: raced.first.id,
  });
  ...
  const [replacement, termination] = await Promise.allSettled([
    redeemRejoinInvite(db, {
      token: racedInvite.token,
      authUserId: racedReplacementAuthUserId,
      displayName: "Replacement race participant",
    }),
  ...
  const racedMemberships = await db
    .select()
    .from(pairMembership)
    .where(eq(pairMembership.pairId, raced.pairId));
  expect(racedMemberships.every((membership) => membership.endedAt !== null)).toBe(true);

  const replacementFirst = await createJoinedPair();
  const replacementFirstInvite = await issueRejoinInvite(db, {
    pairId: replacementFirst.pairId,
    participantId: replacementFirst.first.id,
  });
  const replacementFirstAuthUserId = await createGuestAuthUser("Replacement first participant");
  const replacementFirstResult = await redeemRejoinInvite(db, {
    token: replacementFirstInvite.token,
    authUserId: replacementFirstAuthUserId,
    displayName: "Replacement first participant",
  });
  const replacementFirstMembership = (
    await db
      .select()
      .from(pairMembership)
      .where(
        and(
          eq(pairMembership.pairId, replacementFirst.pairId),
          eq(pairMembership.participantId, replacementFirstResult.participantId),
        ),
      )
  )[0];
  ...
  expect(
    await db
      .select()
      .from(pairMembershipEra)
      .where(eq(pairMembershipEra.pairId, replacementFirst.pairId)),
  ).toHaveLength(2);
  ...
  expect(
    (
      await db
        .select()
        .from(pairMembership)
        .where(eq(pairMembership.pairId, replacementFirst.pairId))
    ).every((membership) => membership.endedAt !== null),
  ).toBe(true);

  const terminationFirst = await createJoinedPair();
  const terminationFirstInvite = await issueRejoinInvite(db, {
    pairId: terminationFirst.pairId,
    participantId: terminationFirst.first.id,
  });
  ...
  expect(
    await capture(
      redeemRejoinInvite(db, {
        token: terminationFirstInvite.token,
        authUserId: lateReplacementAuthUserId,
        displayName: "Late replacement participant",
      }),
    ),
  ).toBeInstanceOf(CloserDomainError);
});
...
test("termination races retain only already-committed Together and candidate actions", async () => {
  ...
  const session = await startTogetherSession(db, {
    pairId: together.pairId,
    participantId: together.first.id,
    category: "deep",
  });
  ...
  const candidate = await startOrResumePrivateConversation(db, {
    pairId: askRace.pairId,
    participantId: askRace.first.id,
    category: "fun",
    clientRequestId: randomUUID(),
  });
  if (candidate.state !== "CANDIDATE") throw new Error("Expected a Private candidate.");
  ...
  const askedRounds = await db
    .select()
    .from(privateRound)
    .where(eq(privateRound.conversationId, candidate.id));
  ...
  const skipped = await startOrResumePrivateConversation(db, {
    pairId: skipFirst.pairId,
    participantId: skipFirst.first.id,
    category: "fun",
    clientRequestId: randomUUID(),
  });
  ...
  await skipPrivateQuestionCandidate(db, {
    pairId: skipFirst.pairId,
    participantId: skipFirst.first.id,
    conversationId: skipped.id,
    candidateId: skipped.candidate.id,
    clientRequestId: randomUUID(),
  });
  ...
  expect(
    (
      await db
        .select()
        .from(privateQuestionCandidate)
        .where(eq(privateQuestionCandidate.id, skipped.candidate.id))
    )[0]?.state,
  ).toBe("skipped");

  const likeFirst = await createJoinedPair();
  const liked = await startOrResumePrivateConversation(db, {
    pairId: likeFirst.pairId,
    participantId: likeFirst.first.id,
    category: "fun",
    clientRequestId: randomUUID(),
  });
  ...
  await setPrivateQuestionCandidateLike(db, {
    pairId: likeFirst.pairId,
    participantId: likeFirst.first.id,
    conversationId: liked.id,
    candidateId: liked.candidate.id,
    liked: true,
  });
  ...
  const skipCandidate = await startOrResumePrivateConversation(db, {
    pairId: skipRace.pairId,
    participantId: skipRace.first.id,
    category: "fun",
    clientRequestId: randomUUID(),
  });
  ...
  const [skip, skipTermination] = await Promise.allSettled([
    skipPrivateQuestionCandidate(db, {
      pairId: skipRace.pairId,
      participantId: skipRace.first.id,
      conversationId: skipCandidate.id,
      candidateId: skipCandidate.candidate.id,
      clientRequestId: randomUUID(),
    }),
  ...
  expect(
    (
      await db
        .select()
        .from(privateQuestionCandidate)
        .where(eq(privateQuestionCandidate.id, skipCandidate.candidate.id))
    )[0]?.state,
  ).toBe(skip.status === "fulfilled" ? "skipped" : "invalidated");
  expect(
    await capture(
      skipPrivateQuestionCandidate(db, {
        pairId: skipRace.pairId,
  ...
  const likeCandidate = await startOrResumePrivateConversation(db, {
    pairId: likeRace.pairId,
    participantId: likeRace.first.id,
    category: "fun",
    clientRequestId: randomUUID(),
  });
  ...
  const [like, likeTermination] = await Promise.allSettled([
    setPrivateQuestionCandidateLike(db, {
      pairId: likeRace.pairId,
      participantId: likeRace.first.id,
      conversationId: likeCandidate.id,
      candidateId: likeCandidate.candidate.id,
      liked: true,
    }),
  ...
  const advancedSession = await startTogetherSession(db, {
    pairId: togetherFirst.pairId,
    participantId: togetherFirst.first.id,
    category: "deep",
  });
  ...
  const togetherHistory = await getFormerEraHistoryForParticipant(db, {
    pairId: togetherFirst.pairId,
    participantId: togetherFirst.first.id,
  });
  expect(
    togetherHistory.eras[0]?.togetherSessions.find((item) => item.id === advancedSession.sessionId)
      ?.questions[0]?.advanced,
  ).toBe(true);
  ...
  expect(
    await db.select().from(privateRound).where(eq(privateRound.id, asked.asked.roundId)),
  ).toHaveLength(1);
});
...
test("termination races preserve committed Decline, Reveal, reaction, and reply history and reject later mutations", async () => {
  ...
  const [decline, declineTermination] = await Promise.allSettled([
    declinePrivateRound(db, {
      pairId: declinedPair.pairId,
      participantId: declinedPair.second.id,
      roundId: declined.asked.roundId,
    }),
  ...
  const declinedHistory = await getFormerEraHistoryForParticipant(db, {
    pairId: declinedPair.pairId,
    participantId: declinedPair.first.id,
  });
  const declinedRound = declinedHistory.eras[0]?.privateConversations
    .flatMap((conversation) => conversation.rounds)
    .find((round) => round.id === declined.asked.roundId);
  expect(declinedRound?.status).toBe(decline.status === "fulfilled" ? "passed" : "answered");

  const declineFirstPair = await createJoinedPair();
  const declinedFirst = await askCurrentCandidate(
    declineFirstPair.pairId,
    declineFirstPair.first.id,
    "deep",
  );
  await declinePrivateRound(db, {
    pairId: declineFirstPair.pairId,
    participantId: declineFirstPair.second.id,
    roundId: declinedFirst.asked.roundId,
  });
  await terminatePair(db, {
    pairId: declineFirstPair.pairId,
    participantId: declineFirstPair.first.id,
  });
  const declineFirstHistory = await getFormerEraHistoryForParticipant(db, {
    pairId: declineFirstPair.pairId,
    participantId: declineFirstPair.first.id,
  });
  expect(
    declineFirstHistory.eras[0]?.privateConversations
      .flatMap((conversation) => conversation.rounds)
      .find((round) => round.id === declinedFirst.asked.roundId)?.status,
  ).toBe("passed");
  ...
  await submitPrivateAnswer(db, {
    pairId: revealPair.pairId,
    participantId: revealPair.first.id,
    roundId: revealed.asked.roundId,
    body: "First reveal answer",
  });
  await submitPrivateAnswer(db, {
    pairId: revealPair.pairId,
    participantId: revealPair.second.id,
    roundId: revealed.asked.roundId,
    body: "Second reveal answer",
  });
  ...
  const revealedFirst = await askCurrentCandidate(
    revealFirstPair.pairId,
    revealFirstPair.first.id,
    "deep",
  );
  await submitPrivateAnswer(db, {
    pairId: revealFirstPair.pairId,
    participantId: revealFirstPair.first.id,
    roundId: revealedFirst.asked.roundId,
    body: "First answer before termination",
  });
  await submitPrivateAnswer(db, {
    pairId: revealFirstPair.pairId,
    participantId: revealFirstPair.second.id,
    roundId: revealedFirst.asked.roundId,
    body: "Second answer before termination",
  });
  ...
  expect(
    (
      await getFormerEraHistoryForParticipant(db, {
        pairId: revealFirstPair.pairId,
        participantId: revealFirstPair.first.id,
      })
    ).eras[0]?.privateConversations
      .flatMap((conversation) => conversation.rounds)
      .find((round) => round.id === revealedFirst.asked.roundId)?.answers,
  ).toHaveLength(2);
  ...
  await submitPrivateAnswer(db, {
    pairId: reactionPair.pairId,
    participantId: reactionPair.first.id,
    roundId: reacted.asked.roundId,
    body: "First reaction answer",
  });
  await submitPrivateAnswer(db, {
    pairId: reactionPair.pairId,
    participantId: reactionPair.second.id,
    roundId: reacted.asked.roundId,
    body: "Second reaction answer",
  });
  await markPrivateRevealViewed(db, {
    pairId: reactionPair.pairId,
    participantId: reactionPair.first.id,
  ...
  const reactionHistory = await getFormerEraHistoryForParticipant(db, {
    pairId: reactionPair.pairId,
    participantId: reactionPair.first.id,
  });
  const reactionRound = reactionHistory.eras[0]?.privateConversations
    .flatMap((conversation) => conversation.rounds)
    .find((round) => round.id === reacted.asked.roundId);
  ...
  await submitPrivateAnswer(db, {
    pairId: replyFirstPair.pairId,
    participantId: replyFirstPair.first.id,
    roundId: replied.asked.roundId,
    body: "First reply answer",
  });
  await submitPrivateAnswer(db, {
    pairId: replyFirstPair.pairId,
    participantId: replyFirstPair.second.id,
    roundId: replied.asked.roundId,
    body: "Second reply answer",
  });
  await markPrivateRevealViewed(db, {
    pairId: replyFirstPair.pairId,
    participantId: replyFirstPair.first.id,
  ...
  await setPrivateReply(db, {
    ...
    participantId: replyFirstPair.first.id,
    roundId: replied.asked.roundId,
    body: "Reply retained in history",
  });
  await terminatePair(db, {
    pairId: replyFirstPair.pairId,
    participantId: replyFirstPair.second.id,
  ...
  expect(
    (
      ...
      .flatMap((conversation) => conversation.rounds)
      .find((round) => round.id === replied.asked.roundId)
      ?.replies.map((reply) => reply.body),
  ).toEqual(["Reply retained in history"]);
  ...
  await submitPrivateAnswer(db, {
    pairId: replyRacePair.pairId,
    participantId: replyRacePair.first.id,
    roundId: replyRace.asked.roundId,
    body: "First concurrent reply answer",
  });
  await submitPrivateAnswer(db, {
    pairId: replyRacePair.pairId,
    participantId: replyRacePair.second.id,
    roundId: replyRace.asked.roundId,
    body: "Second concurrent reply answer",
  });
  await markPrivateRevealViewed(db, {
    pairId: replyRacePair.pairId,
    participantId: replyRacePair.first.id,
  ...
  expect(
    (
      ...
      .flatMap((conversation) => conversation.rounds)
      .find((round) => round.id === replyRace.asked.roundId)?.replies,
  ).toHaveLength(reply.status === "fulfilled" ? 1 : 0);
});
