...
import { joinPairSchema, type JoinPairValues } from "@/contracts/invite/invite.schema";
...
export default function JoinPairForm({
  ...
  async function joinPair(values: JoinPairValues) {
    ...
    try {
      const response = await fetch(
        `/api/${kind === "rejoin" ? "rejoin" : "invites"}/${encodeURIComponent(token)}/redeem`,
        {
          method: "POST",
      ...
      if (!response.ok || !body || typeof body !== "object" || !("pairId" in body)) {
        if (
          ...
          typeof body === "object" &&
          "error" in body &&
          (body as { error?: unknown }).error === "DISPLAY_NAME_INVALID"
        ) {
      ...
      form.setError("root.server", {
        message:
          kind === "rejoin"
            ? "This rejoin link is unavailable. Please try again."
            : "This invitation is unavailable. Please try again.",
      });
    }
  }
  ...
  return (
    <form className="mt-6" onSubmit={form.handleSubmit(joinPair)}>
      {kind === "initial" && initialInvite ? (
        <div className="text-closer-navy rounded-[1.35rem] bg-white/65 p-4 text-sm shadow-[0_8px_24px_rgba(27,33,78,0.08)]">
          <p>
            <strong>Invited by:</strong> {initialInvite.inviterDisplayName}
          </p>
          <p className="mt-2">
            <strong>Space type:</strong>{" "}
            {initialInvite.relationshipType === "partner" ? "Partner" : "Friend"}
          </p>
        </div>
      ) : null}
      ...
      <AsyncButton
        ...
        type="submit"
      >
        {kind === "rejoin" ? "Reconnect" : "Join space"}
      </AsyncButton>
    </form>
  );
}
