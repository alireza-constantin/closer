...
export function TogetherPickerFrame({ children, pairId }: { children: ReactNode; pairId?: string }) {
  return (
    <CloserPageShell className="pt-[18px]">
      <header className="flex min-h-[42px] items-center justify-between gap-3">
        {pairId ? <CloserBackLink href={`/pair/${pairId}`} label="Back to pair home" /> : <Skeleton aria-hidden="true" className="size-9 rounded-[.8rem] bg-white/65" />}
        <CloserWordmark />
