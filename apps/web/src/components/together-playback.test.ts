...
describe("Together loaded-question playback", () => {
  test("uses the documented light, medium, and deep fallback orders from already-loaded pages", () => {
    ...
    expect(chooseBufferedTogetherQuestion(pools, 0)).toMatchObject({
      kind: "question",
      band: "medium",
      question: { questionId: "medium" },
    });
    expect(
      chooseBufferedTogetherQuestion(
        { ...pools, medium: page([]), light: page([question("light")]) },
        2,
      ),
    ).toMatchObject({ kind: "question", band: "light", question: { questionId: "light" } });
    expect(
      chooseBufferedTogetherQuestion(
        { ...pools, light: page([]), medium: page([question("medium")]), deep: page([]) },
        4,
      ),
    ).toMatchObject({ kind: "question", band: "medium", question: { questionId: "medium" } });
  });
  ...
  test("does not treat an empty local buffer with another server page as exhaustion", () => {
    ...
    expect(chooseBufferedTogetherQuestion({ ...pools, light: page([]) }, 0)).toMatchObject({
      kind: "question",
      band: "medium",
    });
  });
  ...
  test("prefetches at the five-question threshold and never after the last page", () => {
    expect(
      shouldPrefetchTogetherQuestionPage(
        page(
          Array.from({ length: 5 }, (_, index) => question(`${index}`)),
          true,
        ),
      ),
    ).toBe(true);
    expect(
      shouldPrefetchTogetherQuestionPage(
        page(
          Array.from({ length: 6 }, (_, index) => question(`${index}`)),
          true,
        ),
      ),
    ).toBe(false);
