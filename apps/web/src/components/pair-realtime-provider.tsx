...
function invalidateForEvent(queryClient: ReturnType<typeof useQueryClient>, pairId: string, type: EventType) {
...
export function createPairRealtimeSubscription(queryClient: ReturnType<typeof useQueryClient>, pairId: string) {
  ...
  const onEvent = (event: MessageEvent<string>) => {
    try {
      ...
      if (!payload || typeof payload !== "object" || !("type" in payload) || !("pairId" in payload)) return;
      ...
      if (type === "pair.changed" || type === "private.changed" || type === "together.changed" || type === "pair.terminated") {
...
export function PairRealtimeProvider({ pairId, children }: { pairId: string; children: React.ReactNode }) {
