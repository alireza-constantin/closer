...
export function JoinInvitationFrame({ children, kind = "initial" }: { children: ReactNode; kind?: "initial" | "rejoin" }) {
  const isRejoin = kind === "rejoin";

  return (
    <OnboardingSurface>
      <OnboardingIcon tone="lavender"><Sparkles aria-hidden="true" /></OnboardingIcon>
      <CloserEyebrow className="mt-4">{isRejoin ? "A way back to Closer" : "An invitation for you"}</CloserEyebrow>
  ...
  return (
    <div aria-hidden="true" className="mt-6">
      <div className="rounded-[1.35rem] bg-white/65 p-4 shadow-[0_8px_24px_rgba(27,33,78,0.08)]">
        <Skeleton className="h-4 w-36 rounded-full bg-closer-navy/10" />
        <Skeleton className="mt-3 h-4 w-28 rounded-full bg-closer-navy/10" />
      </div>
      <Skeleton className="mt-6 h-4 w-20 rounded-full bg-closer-navy/10" />
      <Skeleton className="mt-2 h-11 w-full rounded-[1.05rem] bg-white/90" />
      <Skeleton className="mt-5 h-12 w-full rounded-[1.05rem] bg-closer-coral-soft" />
    </div>
  );
}
