...
function domainErrorResponse(error: unknown) {
  ...
  return Response.json(
    { error: status === 400 ? message : "Unable to create the pair." },
    { status },
  );
}
...
export async function POST(request: Request) {
  ...
  if (
    typeof intendedPersonName !== "string" ||
  ...
  try {
    ...
    if (!resolvedParticipant)
      return Response.json({ error: "Complete onboarding first." }, { status: 409 });
    ...
    return Response.json({
      ...
      intendedPersonName: result.pair.intendedPersonName,
    });
  } catch (error) {
