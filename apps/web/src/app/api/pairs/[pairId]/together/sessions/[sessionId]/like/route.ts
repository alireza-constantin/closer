...
export async function PUT(request: Request, context: { params: Promise<{ pairId: string; sessionId: string }> }) {
  ...
  const currentQuestionId = body && typeof body === "object" && "currentQuestionId" in body ? body.currentQuestionId : undefined;
  if (typeof liked !== "boolean" || (currentQuestionId !== undefined && typeof currentQuestionId !== "string")) {
  ...
  try {
    return Response.json(
      await setTogetherSessionLike(db, { participantId: participant.id, pairId, sessionId, liked, currentQuestionId }),
