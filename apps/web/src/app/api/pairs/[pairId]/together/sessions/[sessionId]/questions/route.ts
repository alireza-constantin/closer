...
export async function GET(request: Request, context: { params: Promise<{ pairId: string; sessionId: string }> }) {
  ...
  if (!band) return Response.json({ error: "Invalid request." }, { status: 400, headers: togetherNoStoreHeaders });
