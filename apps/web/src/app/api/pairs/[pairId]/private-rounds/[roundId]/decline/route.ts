import { db, declinePrivateRound, publishRealtimeEvent } from "@Closer/auth/closer";
...
export async function POST(request: Request, context: { params: Promise<{ pairId: string; roundId: string }> }) {
  ...
  try {
    const result = await declinePrivateRound(db, { participantId: participant.id, pairId, roundId });
