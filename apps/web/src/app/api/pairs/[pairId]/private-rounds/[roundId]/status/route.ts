import { db, getPrivateRoundStatusForParticipant } from "@Closer/auth/closer";

import { privateDomainErrorResponse, requireRequestParticipant, noStoreHeaders } from "@/lib/private-api";

export async function GET(request: Request, context: { params: Promise<{ pairId: string; roundId: string }> }) {
  ...
  try {
    return Response.json(await getPrivateRoundStatusForParticipant(db, { participantId: participant.id, pairId, roundId }), {
