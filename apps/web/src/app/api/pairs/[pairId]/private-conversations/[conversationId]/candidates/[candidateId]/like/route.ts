...
export async function PUT(
  ...
  if (typeof liked !== "boolean") return Response.json({ error: "Invalid request." }, { status: 400, headers: noStoreHeaders });
  const { pairId, conversationId, candidateId } = await context.params;
  try {
    return Response.json(
      await setPrivateQuestionCandidateLike(db, { participantId: participant.id, pairId, conversationId, candidateId, liked }),
