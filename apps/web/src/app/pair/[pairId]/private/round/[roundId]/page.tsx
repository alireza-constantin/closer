...
export default async function PrivateRoundPage({ params }: { params: Promise<{ pairId: string; roundId: string }> }) {
  const { pairId, roundId } = await params;
  const currentParticipant = await getCurrentParticipant();
  if (!currentParticipant) notFound();
  try {
    const view = await getPrivateRoundForParticipant(db, { participantId: currentParticipant.id, pairId, roundId });
