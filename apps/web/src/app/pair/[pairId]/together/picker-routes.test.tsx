...
describe("static Together picker routes", () => {
  test.each([
    ...
    "renders the static %s category picker without a Pair projection",
    async (relationshipType, routeFile) => {
  ...
  test.each(["partner/page.tsx", "friend/page.tsx"])(
    "does not read Pair data in %s",
    async (routeFile) => {
      const source = await Bun.file(join(import.meta.dir, routeFile)).text();
